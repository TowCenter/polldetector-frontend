<script>
  ...
  async function fetchData() {
    ...
    const apiUrl = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' 
      ...
      : 'https://pollfinder-production.up.railway.app/api/polls';
    loading = true;
    error = null;

    try {
      const response = await fetch(`${apiUrl}?date=${dateFilter || todayStr}`);
              ...
              <table class="group-table">
                <thead>
                  <tr>
                    ...
                    <th>End Date</th>
                    <th>Sample Size</th>
                    <th class="sortable" on:click={() => sortGroupedData('confidence')}>
                      Confidence
                      {#if groupSortColumn === 'confidence'}
                        <span class="sort">{groupSortDirection === 'asc' ? '▲' : '▼'}</span>
                      {/if}
                    </th>
                  </tr>
